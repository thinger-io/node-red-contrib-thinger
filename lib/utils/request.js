...
class Request {
    ...
    constructor(token, maxSockets, requestTimeout) {
        ...
        if ( typeof maxSockets !== 'undefined' && maxSockets !== "" && maxSockets !== "0" ) {
            ...
            options.maxSockets = Infinity;
        } else { // default 5 sockets
    ...
    request(url, method, data, external = false) {
        ...
        // Remove authorization header for external requests
        if ( external ) {
          ...
          const req = adapterFor(url).request(url, options, (res) => {
            ...
            res.on('end', () => {
              ...
              if (res.statusCode == 404) {
                let err = new Error(`The queried resource is not found.`);
                err.status = res.statusCode;
                return reject(err);
              }
              if (res.statusCode >= 400) {
