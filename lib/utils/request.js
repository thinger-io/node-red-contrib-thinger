          ...
          const req = adapterFor(url).request(url, options, (res) => {
            ...
            res.on('end', () => {
              ...
              if (res.headers.hasOwnProperty('content-type')) { // res.headers is a class property, not an object property
                message = res.headers['content-type'].startsWith('application/json') ? JSON.parse(message) : message;
              }
              ...
              if (res.statusCode == 401) {
                let err = new Error(`Invalid JWT. Check Thinger server configuration.`);
                err.status = res.statusCode;
                return reject(err);
              }
