  ...
  function AssetIteratorNode(config) {
    ...
    function handleResponse(msg, res) {
      for (const i in res) {
        ...
        if (res.length > 1) {
          // if more than one asset create new message with its own id
    ...
    node.on("input", async function (msg, send, done) {
      try {
        ...
        node.rate =
          rateUnits === "s" ? parseInt(node.rate) * 1000 : parseInt(node.rate);
        ...
        if (typeof server.request === "function") {
          const assetsUrl = `${server.config.ssl ? "https://" : "http://"}${
            server.config.host
          }/v1/server/assets`;
          ...
          try {
            ...
            const url = `${server.config.ssl ? "https://" : "http://"}${
              server.config.host
            }/${path}?count=${count}&${queryParametersString}`;
