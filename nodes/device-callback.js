module.exports = function (RED) {
  ...
  async function provisionDevice(
    ...
    let url = `${server.config.ssl ? "https://" : "http://"}${
      server.config.host
    }/v1/users/${server.config.username}/devices`;

    let res = await server.request(node, url, "POST", data);
    if (!res) {
      return new Error(
        `Could not create device on device callback auto provisioning`
      );
    }
    ...
    url = `${server.config.ssl ? "https://" : "http://"}${
      server.config.host
    }/v3/users/${server.config.username}/devices/${prefix}${device}/callback`;
    res = await server.request(node, url, "PUT", dataAction);
    if (!res) {
      return new Error(
        `Could not configure device callback to write bucket action`
      );
    }
  }
  ...
  async function provisionBucket(
    ...
    const url = `${server.config.ssl ? "https://" : "http://"}${
      server.config.host
    }/v1/users/${server.config.username}/buckets`;

    try {
      await server.request(node, url, "POST", data); // There is no response
    } catch (e) {
      return new Error(
        `Could not create bucket on device callback auto provisioning`
      );
    }
  }
    ...
    node.on("input", async function (msg, send, done) {
      try {
        ...
        if (typeof server.request === "function") {
          ...
          }/devices/${prefix}${device}/callback`;
