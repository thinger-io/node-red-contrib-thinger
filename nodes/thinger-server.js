...
module.exports = function (RED) {
  ...
  function ThingerServerNode(configa) {
    ...
    node.openWebsocket = function (
      ...
      const url = `${config.ssl ? "wss://" : "ws://"}${config.host}/v1/users/${
        config.username
      }${path}?authorization=${config.token}`;
      ...
      return ThingerRequest.request(url, method, data, external)
        .then((res) => {
          ...
          if (typeof data !== "undefined") {
            if (Buffer.isBuffer(data)) body = data.toString("hex");
            else if (typeof data === "object") body = JSON.stringify(data);
          } else body = "";
          caller.log(
            `${res.status} ${method} ${url} ${Utils.truncateString(body, 1024)}`
          );
          return res;
        })
        .catch((err) => {
          ...
          caller.log(
            `${err.status} ${method} ${url} ${Utils.truncateString(body, 1024)}`
          );
          throw err; // handle from parent caller
        });
    };

    node.unRegisterDeviceResourceListener = function (
      deviceId,
      resourceId,
      node
    ) {
    ...
    node.registerDeviceResourceListener = function (
      deviceId,
      resourceId,
      interval,
      node
      ...
      if (Object.keys(resource.listeners).length) {
        ...
        for (let id in resource.listeners) {
          ...
          if (listener.interval !== resource.interval) {
            listener.skipMeasures =
              listener.interval > 0 ? listener.interval / resource.interval : 0;
        ...
        if (
          !device.wss ||
          device.wss.readyState === device.wss.CLOSING ||
          device.wss.readyState === device.wss.CLOSED
        ) {
    ...
    function getDeviceWebsocket(deviceId) {
      const url = `${config.ssl ? "wss://" : "ws://"}${config.host}/v3/users/${
        config.username
      }/devices/${deviceId}/resources?authorization=${token}`;
  ...
  RED.httpNode.get("/assets/:asset", async function (req, res) {
    ...
    if (!req.query.svr_id) {
      // If no server node has been selected return empty
    ...
    try {
      const url = `${server.config.ssl ? "https://" : "http://"}${
        server.config.host
      }/v1/users/${server.config.username}/${req.params.asset}?${filter}`;
      res.json((await server.request(server, url, "GET")).payload);
    } catch (err) {
      res.sendStatus(500);
      if (server) server.error(`assets.failed ${err.toString()}`);
    }
  });
  ...
  RED.httpNode.get("/assets/:category/:asset", async function (req, res) {
    ...
    try {
      const url = `${server.config.ssl ? "https://" : "http://"}${
        server.config.host
      }/v1/users/${server.config.username}/${req.params.category}/${
        req.params.asset
      }?${filter}}`;
      res.json((await server.request(server, url, "GET")).payload);
    } catch (err) {
  ...
  RED.httpNode.get(
    "/assets/:asset/:asset_id/:properties",
    async function (req, res) {
      ...
      try {
        const url = `${server.config.ssl ? "https://" : "http://"}${
          server.config.host
        }/${apiVersion}/users/${server.config.username}/${req.params.asset}/${
          req.params.asset_id
        }/${req.params.properties}?${filter}`;
        res.json((await server.request(server, url, "GET")).payload);
      } catch (err) {
        res.sendStatus(500);
  ...
  RED.httpNode.get(
    "/assets/:asset/:asset_id/:properties/:property_id",
    async function (req, res) {
      const apiVersion = req.params.asset === "devices" ? "v3" : "v1";
      ...
      try {
        const url = `${server.config.ssl ? "https://" : "http://"}${
          ...
          req.params.asset_id
        }/${req.params.properties}/${req.params.property_id}?${filter}`;
        res.json((await server.request(server, url, "GET")).payload);
      } catch (err) {
        res.sendStatus(500);
        server.error(`properties.failed ${err.toString()}`);
  ...
  RED.httpNode.get("/users/user", async function (req, res) {
    if (
      typeof req.query.svr_id === "undefined" ||
      !req.query.svr_id ||
      req.query.svr_id === ""
    ) {
      // If no server node has been selected return empty
    ...
    try {
      const url = `${server.config.ssl ? "https://" : "http://"}${
        server.config.host
      }/v1/users/${server.config.username}`;
      res.json((await server.request(server, url, "GET")).payload);
    } catch (err) {
      res.sendStatus(500);
  ...
  RED.httpNode.get("/server/:resource", async function (req, res) {
    ...
    try {
      const method = "GET";
      const url = `${ssl ? "https://" : "http://"}${host}/v1/server/${
        req.params.resource
      }`;
      let data = {};
