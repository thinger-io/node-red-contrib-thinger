module.exports = function(RED) {
    ...
    function subscribeAndExecute(server, node, msg, send, done, bucket, body, dataReturn) {
            ...
            if ( export_file === payload.file ) {
                ...
                if ( dataReturn === "content" ) {
                    try {
                        ...
                        payload.content = (await server.request(node, payload.url, 'GET', null, external)).payload;
                    } catch ( e ) {
                        delete e.stack;
                        msg.payload = {};
                        msg.payload.bucket = bucket;
        ...
        if ( typeof server.openWebsocket === "function" )
            wss = server.openWebsocket(node, "/events", on_open, on_message, on_close, on_error);
        else
    ...
    function exportBucket(server, node, bucket, body, done) {
        ...
        if (typeof server.request === "function")
            return server.request(node, url, method, body)
            .then(function(res) {
    ...
    function BucketExportNode(config) {
        ...
        node.on("input",function(msg, send, done) {
            ...
            */
            ...
            try {
                subscribeAndExecute(server, node, msg, send, done, bucket, body, dataReturn);
            } catch (e) {
                delete e.stack;
