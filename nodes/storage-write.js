  ...
  function StorageWriteNode(config) {
    ...
    node.on("input", async function (msg, _send, done) {
      try {
        ...
        let appendNewLine =
          msg.append_new_line || config.appendNewLine || false;
        let createDir = msg.create_dir || config.createDir || false;

        const method = "GET";
        const url = `${server.config.ssl ? "https://" : "http://"}${
          server.config.host
        }/v1/users/${server.config.username}/storages/${storage}/files`;
        ...
        if (typeof server.request === "function") {
          try {
            ...
            if (files.length === 1 && file[0].type === "directory")
              // file exists
              throw new Error("File is a directory");
            else if (files.length > 1) {
              // file might have the same name as a folder
              throw new Error("File is a directory");
              ...
              if (createDir) {
                ...
                while (i <= fileDepth + 1) {
                  ...
                  let exists =
                    res.payload.filter((e) => e.path.startsWith(path.join("/")))
                      .length > 0;
                  if (!exists) {
            ...
            if (appendNewLine) {
              if (Buffer.isBuffer(msg.payload))
                newContent = Buffer.concat([msg.payload, Buffer.from("\n")]);
              else if (Utils.isBase64(msg.payload))
                newContent =
            ...
            try {
              if (action === "append") {
                let content = (
                  await server.request(node, `${url}/${file}`, method)
                ).payload;
                if (Buffer.isBuffer(content)) {
                  if (Buffer.isBuffer(newContent))
                    msg.payload = Buffer.concat([content, newContent]);
                  else
                    msg.payload = `${content.toString("utf-8")}${newContent}`;
                ...
                msg.payload = await server.request(
                  node,
                  `${url}/${file}`,
                  "PUT",
                  msg.payload
