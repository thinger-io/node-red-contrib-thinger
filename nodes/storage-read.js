      ...
      try {
        ...
        if (typeof server.request === "function") {
          ...
          try {
            ...
            let files = res.payload.filter(
              (e) => e.path.startsWith(`${file}/`) || e.path === file
            );
            const regex = /[/]/g;
            ...
            if (!recursive) {
              files = files.filter(
                (e) => (e.path.match(regex) || []).length <= fileDepth
              );
            } else {
              if (maxDepth != -1)
                files = files.filter(
                  (e) =>
                    (e.path.match(regex) || []).length >=
                      fileDepth + minDepth &&
                    (e.path.match(regex) || []).length <=
            ...
            if (files.length === 1)
              if (data === "content") msg.payload = files[0].content;
              else msg.payload = files[0];
            else msg.payload = files;
