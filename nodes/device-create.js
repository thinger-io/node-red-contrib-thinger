  ...
  function DeviceCreateNode(config) {
    ...
    node.on("input", async function (msg, send, done) {
      try {
        ...
        if (deviceType !== undefined) data.type = deviceType;
        data.device = config.deviceId || msg.device;

        let credentials;
        if (
          this.credentials &&
          this.credentials.hasOwnProperty("deviceCredentials")
        ) {
        ...
        let projects =
          config.projects && config.projects !== "[]"
            ? RED.util.evaluateNodeProperty(config.projects, "json", node)
            : msg.projects;
        if (typeof projects === "object" && projects.length > 0) {
          projects = projects.map(
            (project) => `${server.config.username}@${project}`
          ); // body needs username
          if (Utils.isTemplated(projects))
            projects = Utils.mustacheRender(projects, msg);
        }
        ...
        if (typeof server.request === "function") {
          ...
          try {
            try {
              ...
              if (!res || res.status !== 200) exists = false;
            }
