module.exports = function (RED) {
  ...
  function readBucket(server, node, bucket, queryParameters, limit, result) {
    ...
    }/buckets/${bucket}/data?${queryParametersString}`;

    // Recursive call to readBucket until done
    if (typeof server.request === "function")
      return server.request(node, url, method).then(function (res) {
        if (!result) {
          result = [];
        }
        ...
        if (limit != 0 && res.payload.length == 1000) {
          // There is more
  ...
  function BucketReadNode(config) {
    ...
    node.on("input", function (msg, send, done) {
      ...
      queryParameters.set(
        "agg_type",
        config.aggregationType || msg.aggregation_type
      );
      ...
      readBucket(server, node, bucket, queryParameters, limit, result)
        .then(function (data) {
          if (isSimpleSorting) {
            // sort last N items asc if needed
